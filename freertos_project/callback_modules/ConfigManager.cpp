...
//---------------- Array em RAM ----------------
size_t ConfigManager::op_GetStepCount() {
    return stepCount_;
}

bool ConfigManager::op_PushStep(int temp, uint32_t duration) {
...
int ConfigManager::op_GetTemperature(size_t idx) {
    return (idx < stepCount_) ? temps_[idx] : nanf("");
}

uint32_t ConfigManager::op_GetDuration(size_t idx) {
    return (idx < stepCount_) ? durations_[idx] : 0;
}

void ConfigManager::op_ClearSteps() {
    stepCount_ = 0;
